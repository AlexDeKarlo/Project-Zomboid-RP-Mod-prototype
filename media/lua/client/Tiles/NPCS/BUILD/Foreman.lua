...
local function OnFillWorldObjectContextMenu(player, context, worldObjects, test)
    for i, v in ipairs(worldObjects) do
		...
		if(spriteName2 == "NPCS_1") then
            context:addOptionOnTop(getText("ContextMenu_ProjectRP_FOREMAN"), player, function() ClickFunction(getSpecificPlayer(player),v) end,'Arg')
			break
		end
    end
end
