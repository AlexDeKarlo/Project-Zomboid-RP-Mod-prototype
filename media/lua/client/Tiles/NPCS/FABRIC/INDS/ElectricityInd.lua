...
local function OnFillWorldObjectContextMenu(player, context, worldObjects, test)
    for i, v in ipairs(worldObjects) do
		...
		if(spriteName2 == "industry_02_46" or spriteName2 == "industry_02_47") then
            context:addOptionOnTop(getText("IGUI_RPCRAFT_ELECTRICITY"), player, function() ClickFunction(getSpecificPlayer(player),v) end,'Arg')
			break
		end
    end
end
